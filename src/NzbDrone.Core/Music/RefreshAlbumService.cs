...
namespace NzbDrone.Core.Music
{
    ...
    public class RefreshAlbumService : RefreshEntityServiceBase<Album, AlbumRelease>, IRefreshAlbumService, IExecute<RefreshAlbumCommand>
    {
        ...
        protected override UpdateResult MergeEntity(Album local, Album target, Album remote)
        {
            ...
            var releases = _releaseService.GetReleasesByAlbum(local.Id);
            _logger.Trace($"Moving {releases.Count} releases from {local} to {remote}");

            releases.ForEach(x => x.AlbumId = target.Id);
            _releaseService.UpdateMany(releases);
        ...
        {
            return _releaseService.GetReleasesForRefresh(entity.Id,
        ...
        protected override bool RefreshChildren(SortedChildren localChildren, List<AlbumRelease> remoteChildren, bool forceChildRefresh, bool forceUpdateFileTags)
        {
            ...
            var monitored = localChildren.Future.FirstOrDefault(x => x.Monitored);

            refreshList.ForEach(x => x.Monitored = false);
            if (monitored != null)
            {
            ...
            {
                localChildren.Future.OrderByDescending(x => x.TrackCount).First().Monitored = true;
            }
